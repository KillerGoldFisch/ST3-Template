...
class DummyResponse():
    ...
    def write(self, data, escape=False):
        if not escape:
...
def render(content="hello world",
    ...
    if context and 'response' in context:
        ...
        context['response'].body = StringIO() #StringIO.StringIO()
    else:
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) != 2:
        print("Usage: template.py <teplatefile>")
        exit()

    import sys
    filename = sys.argv[1]

    outtext = render(filename=filename)
    lines = outtext.replace("\r", "").split("\n")
    for l in lines:
        print(l)
